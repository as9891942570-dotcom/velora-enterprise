...
async def _issue_customer_tokens(db: AsyncSession, user: User, response: Response) -> TokenResponse:
...
async def _issue_admin_tokens(db: AsyncSession, user: User, response: Response) -> TokenResponse:
    access_token = create_access_token(
        str(user.id),
        extra_claims={"role": user.role.value},
    )
...
async def _revoke_refresh_cookie(db: AsyncSession, raw_token: str | None) -> None:
    if not raw_token:
        return
...
async def register(
    ...
    if session_id:
        await cart_service.merge_guest_cart(db, user.id, session_id)

    return await _issue_customer_tokens(db, user, response)
...
async def admin_login(
    ...
    return await _issue_admin_tokens(db, user, response)
...
async def logout(
    ...
    await _revoke_refresh_cookie(db, request.cookies.get(REFRESH_COOKIE_NAME))
    clear_refresh_cookie(response)
...
async def admin_logout(
    ...
    await _revoke_refresh_cookie(db, request.cookies.get(ADMIN_REFRESH_COOKIE_NAME))
    clear_admin_refresh_cookie(response)
    return MessageResponse(message="Logged out successfully")
